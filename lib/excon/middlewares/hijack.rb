module Excon
  module Middleware
    # Hijack is an Excon middleware which parses response headers and then yields the underlying
    # TCP socket for raw TCP communication (used to attach to STDIN of containers).
    class Hijack < Base
      def response_call(datum)
        if datum[:hijack_block]
          # Need to process the response headers here rather than in Excon::Middleware::ResponseParser
          # as the response parser will block trying to read the body.
